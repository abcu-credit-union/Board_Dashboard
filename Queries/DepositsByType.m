...
"SELECT
    ...
    WH_ACCTCOMMON.NOTEBAL ""Account Balance"",
    ...
    COALESCE(CEIL(MONTHS_BETWEEN(ACCT.DATEMAT, WH_ACCTCOMMON.EFFDATE)), 0) AS RemainingAmortization,
...
*/
    ...
    #"Transformed GLs" = Table.FromRecords(
        Table.TransformRows(#"Added Old Gls", (r) => Record.TransformFields(r,
            {{"GL Number", each if Date.From(r[EFFDATE]) <= Date.FromText("3/31/2019") and r[Old GL] <> null then
                r[Old GL]
            else _}}))),
...
*/
    #"Adjusted Account Balances" = Table.TransformColumns(#"Added GL Codes",
        ...
        */
        ...
        #"Grouped Rows" = Table.Group(#"Adjusted Account Balances", {"EFFDATE", "FR2900Code", "Cost Center"},
...
*/
    #"Added DNA GL Balances" = Table.ExpandTableColumn(
        Table.NestedJoin(#"Adjusted Account Balances", {"EFFDATE", "FR2900Code", "Cost Center"},
            #"Grouped Rows", {"EFFDATE", "FR2900Code", "Cost Center"},
    ...
    "Prod Bal", {"Sum"}, {"DNA Balance"}),
        ...
        */
        #"Grouped FR2900 Codes" = Table.Group(FASQuery, {"MonthYYYYMM", "FR2900Code", "OrgShortName"},
            {"Balance", each List.Sum([YTDBal]), type number}),
/*
Add a date column with the same format as FAS dates
*/
    #"Added FAS Date Column" = Table.AddColumn(#"Added DNA GL Balances", "FAS Dates",
        each Date.ToText(Date.From([EFFDATE]), "yyyyMM"), type text),
/*
Adding FAS balances back to the main query
*/
    #"Added FR Balances" = Table.ExpandTableColumn(
        Table.NestedJoin(#"Added FAS Date Column", {"FAS Dates", "FR2900Code", "Cost Center"},
            #"Grouped FR2900 Codes", {"MonthYYYYMM", "FR2900Code", "OrgShortName"},
...
*/
    #"Added FP Balances" = Table.AddColumn(#"Filtered Rows", "FP Balance",
        ...
        #"Grouped Entities" = Table.Group(#"Added FP Balances", {"EFFDATE", "Entity", "Source DB"},
    ...
    #"Joined Deposit Class" = Table.ExpandTableColumn(
        Table.NestedJoin(#"Added FP Balances", {"EFFDATE", "Entity", "Source DB"},
            #"Added Deposit Class", {"EFFDATE", "Entity", "Source DB"},
    ...
    #"Grouped Rows1" = Table.Group(#"Checked for Wholesale Deposits", {"Deposit Type"},
        {{"Balance", each List.Sum([FP Balance]), type number}})
